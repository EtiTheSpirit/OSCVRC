...
namespace OSCVRC {
	...
	public class VRCAvatarParameterOSCInterface : IDisposable {
		...
		private static byte[] NewOSCPacket<T>(string parameterName) where T : struct {
			byte[] paramNameBytes = OSCDataManager.GetOSCString($"{AVATAR_PARAMETER_PATH_PREFIX}/{parameterName}");
		...
		public void SetAvatarParameter(string parameterName, float value) {
			...
			byte[] valueBytes = OSCDataManager.GetBigEndianBytesOf(value);
			AppendToEnd(packet, valueBytes);
			OSCDataManager.Pad(ref packet);
			_sender.Send(packet);
			if (LogActions) LogUtil.LogSend(parameterName, value);
		}
		...
		private void HandleParameters(byte[] buf) {
			string newReceivedInfo = OSCDataManager.RawByteArrayToString(buf);
			newReceivedInfo = OSCDataManager.RawByteArrayToString(_overflowBuffer.ToArray()) + newReceivedInfo; // Put anything missing onto the end here.
			...
			_overflowBuffer.AddRange(OSCDataManager.StringToRawByteArray(newReceivedInfo));
		}
		...
		private bool HandleMatch(Match match, ref string newReceivedInfo) {
			...
			if (parameterName.StartsWith("/avatar/parameters/")) {
				parameterName = parameterName[19..];
			} else {
				goto EXIT;
			}
			...
			byte[] rawValue = OSCDataManager.StringToRawByteArray(rawValueStr);
			if (type == "T" || type == "F") {
				bool value = type == "T";
				bool changed = PutIfChanged(parameterName, new Variant<int, float, bool>(value));
				if (changed) {
					if (LogActions) LogUtil.LogReceive(parameterName, null, value);
				...
				int value = OSCDataManager.GetIntFromBigEndian(rawValue);
				bool changed = PutIfChanged(parameterName, new Variant<int, float, bool>(value));
				if (changed) {
					if (LogActions) LogUtil.LogReceive(parameterName, rawValue, value);
					ParameterChanged?.Invoke(parameterName, value);
				...
				float value = OSCDataManager.GetFloatFromBigEndian(rawValue);
				bool changed = PutIfChanged(parameterName, new Variant<int, float, bool>(value));
				if (changed) {
					if (LogActions) LogUtil.LogReceive(parameterName, rawValue, value);
					ParameterChanged?.Invoke(parameterName, value);
...
EXIT:
			...
			if (len == newReceivedInfo.Length) {
				newReceivedInfo = string.Empty;
			} else {
				newReceivedInfo = newReceivedInfo[len..]; // Skip ahead
			}
